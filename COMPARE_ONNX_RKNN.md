...
| 结论文案 | 什么时候算过 | 含义 |
|----------|--------------|------|
| **转换可用** | match_rate≥0.95 且 mean_iou≥0.90 且 mean\|dconf\|≤0.05 | 直接过，可进入下一环或部署 |
| **i8 可接受** | match_rate≥0.85 且 mean_iou≥0.80（但不满足上面） | 勉强过，建议抽看 FAIL 图 |
| **偏差较大** | 不满足上面 | 不过，要查导出/量化/后处理 |

**三段链路各自看自己的结论**：`.pt`↔`.onnx` 期望「**导出可用**」（见 `COMPARE_PT_ONNX.md`）；本环节（`.onnx`↔`.rknn`）与 `.pt`↔`.rknn` 期望「**转换可用**」——FP 期望「转换可用」，i8 最好也是「转换可用」，至少「i8 可接受」。本项目 safety_helmet_all：三段都是「可用」→ 整体通过。
...
| 指标 | 作用 |
|------|------|
| recall_vs_onnx | 越大越好，看漏检 |
| extra_rate | 越小越好，看多检 |
| high_conf recall | 主目标（高分框）是否稳，最好接近 1 |
| mean_cosine / sig_cos_min | 张量层，导出/FP 常接近 1；i8 看 sig_cos，别被背景 cos 吓到 |
| PASS/WARN/FAIL 张数 | 只用来扫图，不要只看这个过不过 |

**一句话记法**：先看 match_rate，再看 mean_iou，最后看 mean\|dconf\|；FP 三项达标 = 转换可用，i8 到「i8 可接受」也基本能部署；「偏差较大」分段排查，`score抹零` 直接重转。
