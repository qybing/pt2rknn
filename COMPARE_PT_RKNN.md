...
| 结论 | 条件 | 含义 |
|------|------|------|
| 转换可用 | `match_rate≥0.95` 且 `mean_iou≥0.90` 且 `mean\|dconf\|≤0.05` | 和 `.pt` 高度一致 |
| i8 可接受 | `match_rate≥0.85` 且 `mean_iou≥0.80`（但不满足「转换可用」） | 有量化漂移，抽看 FAIL 图 |
| 偏差较大 | 不满足以上 | 查导出头、预处理、后处理或量化 |

读结果时：**优先看 `match_rate`、`mean_iou`、`mean|dconf|`**，PASS/WARN/FAIL 张数只作辅助。
