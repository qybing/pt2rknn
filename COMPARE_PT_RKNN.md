...
主看 **match_rate + mean_iou**；`mean|dconf|` 与 `high_conf recall` 为辅。以下针对 **检测 PT↔RKNN 一致性**（非相对 GT 的 mAP）。

**A. FP RKNN（几乎无损）**

| 档位 | match_rate | mean_iou | mean\|dconf\| |
|------|------------|----------|---------------|
| 优 | ≥0.98 | ≥0.95 | ≤0.03 |
| 可用 | ≥0.95 | ≥0.90 | ≤0.05 |

**B. INT8 RKNN（本批模型均为 i8，推荐主用）**

| 档位 | match_rate | mean_iou | mean\|dconf\| | 建议同时看 |
|------|------------|----------|---------------|------------|
| **优 / 可用** | ≥0.95 | ≥0.90 | ≤**0.10** | high_conf recall ≥0.98 |
| **可接受** | ≥0.90 | ≥0.85 | ≤**0.15** | high_conf recall ≥0.95 |
| **偏差较大** | 不满足以上 | | | 先查预处理 / 导出头 / 校准 |

相对脚本内置规则的调整要点：

1. `match_rate` / `mean_iou`：可用档与脚本接近，可接受档略收紧到 0.90 / 0.85（比脚本的 0.85 / 0.80 更稳妥）。  
2. **`mean|dconf|`：i8「可用」从 0.05 放宽到 0.10**（脚本对 i8 过严；手机仅因 0.058 进不了「可用」不合理）。  
3. 有条件时再补：相对 PT 的 **mAP 掉点 < 1~2pt** 为优。
