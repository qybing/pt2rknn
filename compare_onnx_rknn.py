...
类别名在脚本顶部 CLASS_NAMES 中固化，nc = len(CLASS_NAMES)。换数据集时改这一处即可。
"""
...
# 类别名固化在此；顺序须与训练/导出一致。类别数 nc = len(CLASS_NAMES)
# 换数据集时改 CLASS_NAMES 指向下面某一组即可。
HELMET_CLASS_NAMES: tuple[str, ...] = ("head", "helmet")
...
def class_name_map(class_names: tuple[str, ...] | list[str] = CLASS_NAMES) -> dict[int, str]:
    """由固化类别名生成 id→名称；nc 取列表长度。

    Args:
        class_names: 类别名列表，顺序与训练一致。
    ...
    Raises:
        ValueError: 类别名为空时。
    """
    names = tuple(str(n).strip() for n in class_names if str(n).strip())
    if not names:
        raise ValueError("CLASS_NAMES 不能为空，请在脚本顶部填写类别名")
...
def main() -> int:
    ...
    names = class_name_map(CLASS_NAMES)
    nc = len(names)
